...
public sealed partial class RankPage : Page
{
    ...
    private RankComic? lastClickedComic = null;
    ...
    private async void c_GridView_RankList_Loaded(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                c_GridView_RankList.ScrollIntoView(lastClickedComic);
                c_GridView_RankList.UpdateLayout();
                var ani = ConnectedAnimationService.GetForCurrentView().GetAnimation("ComicCoverBackAnimation");
                if (ani != null)
                {
                    ani.Configuration = new BasicConnectedAnimationConfiguration();
                    await c_GridView_RankList.TryStartConnectedAnimationAsync(ani, lastClickedComic, "c_Image_ComicCover");
                }
            }
        }
        catch (Exception ex)
    ...
    private void c_GridView_RankList_ItemClick(object sender, ItemClickEventArgs e)
    {
        try
        {
            if (e.ClickedItem is RankComic comic)
            {
                lastClickedComic = comic;
                var ani = c_GridView_RankList.PrepareConnectedAnimation("ComicCoverAnimation", comic, "c_Image_ComicCover");
                ani.Configuration = new BasicConnectedAnimationConfiguration();
