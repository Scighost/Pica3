...
{
    internal abstract class PicaFileCacheBase<T>
    {
        ...
        public async Task ClearAsync()
        {
            ...
            await PicaFileCacheBase<T>.InternalClearAsync(files).ConfigureAwait(false);
        ...
        public async Task RemoveExpiredAsync(TimeSpan? duration = null)
        {
            ...
            await PicaFileCacheBase<T>.InternalClearAsync(filesToDelete).ConfigureAwait(false);
        ...
        protected virtual async Task<T?> DownloadFileAsync(Uri uri, StorageFile baseFile, bool preCacheOnly, CancellationToken cancellationToken, List<KeyValuePair<string, object>>? initializerKeyValues = null)
        {
            T? instance = default;
