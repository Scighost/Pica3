...
public partial class ImageExBase
{
    ...
    public bool RetryOnLoaded
    {
        get { return (bool)GetValue(RetryOnLoadedProperty); }
        set { SetValue(RetryOnLoadedProperty, value); }
    }

    // Using a DependencyProperty as the backing store for RetryOnLoaded.  This enables animation, styling, binding, etc...
    public static readonly DependencyProperty RetryOnLoadedProperty =
        DependencyProperty.Register("RetryOnLoaded", typeof(bool), typeof(ImageExBase), new PropertyMetadata(false, RetryOnLoadedChanged));


    private static void RetryOnLoadedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is ImageExBase control && control.EnableLazyLoading)
        {
            if ((bool)e.NewValue)
            {
                control.Loaded += control.RetrySetSourceOnLoaded;
            }
            else
            {
                control.Loaded -= control.RetrySetSourceOnLoaded;
    ...
    private void RetrySetSourceOnLoaded(object sender, RoutedEventArgs e)
    {
