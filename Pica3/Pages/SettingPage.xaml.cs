...
#if Dev
    public string AppVersion => "开发版 " + typeof(App).Assembly.GetName().Version?.ToString() ?? "-";
#else
    public string AppVersion => "当前版本 " + typeof(App).Assembly.GetName().Version?.ToString() ?? "-";
#endif
            ...
            if (result.Headers.TryGetValues("x-oss-meta-version", out var values))
            {
                if (Version.TryParse(values.FirstOrDefault(), out var version))
                {
                    if (version > typeof(App).Assembly.GetName().Version)
                    {
                        ...
                        {
                            Title = $"新版本 {version}",
                            Content = "最新生成版不是正式发布版，可能存在功能缺失，Bug 频出的问题。\n下载过程中可继续使用，关闭应用后自动更新。",
                            IsPrimaryButtonEnabled = true,
